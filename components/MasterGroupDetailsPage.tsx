...
import { Page, MasterGroup, StructuringOperation, Event, MasterGroupContact, OperationRisk } from '../types';
...
const MasterGroupDetailsPage: React.FC<MasterGroupDetailsPageProps> = ({ masterGroupId, onNavigate, apiUrl, showToast, pushToGenericQueue }) => {
  ...
  const [contactToEdit, setContactToEdit] = useState<MasterGroupContact | null>(null);
  ...
  const handleSaveStructuringOperation = async (data: Omit<StructuringOperation, 'id' | 'masterGroupId' | 'masterGroupName'>) => {
    try {
      ...
      const url = isEditing 
        ...
        : `${apiUrl}/api/master-groups/${masterGroupId}/structuring-operations`;
  ...
  const handleSaveContact = async (data: Omit<MasterGroupContact, 'id' | 'masterGroupId'>) => {
    try {
      const isEditing = !!contactToEdit;
      const url = isEditing 
        ? `${apiUrl}/api/master-groups/${masterGroupId}/contacts/${contactToEdit.id}`
        : `${apiUrl}/api/master-groups/${masterGroupId}/contacts`;
      ...
      showToast(`Contato ${isEditing ? 'atualizado' : 'criado'} com sucesso`, 'success');
    ...
    <div className="space-y-6 text-gray-900 dark:text-gray-100">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="space-y-4">
              ...
              {(!masterGroup.operations || masterGroup.operations.length === 0) && (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Contatos</h2>
              <button 
                onClick={() => {
                  setContactToEdit(null);
            ...
            <div className="space-y-4">
              {masterGroup.contacts?.map(contact => (
                ...
                >
                  ...
                  {contact.email && <p className="text-sm text-blue-600 dark:text-blue-400">{contact.email}</p>}
